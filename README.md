<p align="center">
  ...
  <img src="https://img.shields.io/badge/plugins-348-000000" alt="Plugin count" />
...
**348 plugins from the Cursor marketplace, organized by the job each one does, and for every one
...
- **Full catalog:** all 348 Cursor plugins with the complete portability and sign-in matrix in [CATALOG.md](CATALOG.md)
...
<!-- nosignin:start -->
**3. Sign in only if the entry says so.** 112 of the 348 have nothing to sign in to. The rest say
...
loads in a second agent. **143 of the 348 listings do exactly that. 201 are Cursor and nothing
...
|---|---:|---|
| Claude Code | 128 | `.claude-plugin/plugin.json` |
| Codex | 80 | `.codex-plugin/plugin.json` |
| The Agent Plugins standard | 42 | `plugin.json` at the plugin root |
| Grok Bot | 19 | `.grok-plugin/plugin.json` |
...
<!-- signin:start -->
290 of the 348 plugins bring an MCP server. The question that decides whether you install one
...
|---|---:|
| OAuth sign-in, click once and you are in | 197 |
| Nothing to sign in to | 112 |
| Paste a token or an API key first | 17 |
| Points at your own instance, so you configure the URL | 16 |
